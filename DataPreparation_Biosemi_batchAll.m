...
% this code batch processes all subjects
...
% obtain a list of unique patient ids
allSub = unique(trialInfo(:,"ParticipantID"));
allSub= allSub.ParticipantID;
allSub = allSub(~cellfun(@isempty, allSub));
allSub = strrep(allSub, ' ', '');% Remove spaces within strings
allSub = unique(allSub);

for s = 1: size(allSub,1)
    SubID = allSub{s};
